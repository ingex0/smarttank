...
namespace SmartTank.Draw.UI.Controls
{
    public class Listbox2 : Control
    {
        ...
        #region Consturtion
        public Listbox2( string name, Vector2 position, Point size, Color backcolor, Color forecolor )//, Font font)
        {
        ...
        {
            listBackground = new Texture2D( BaseGame.Device, size.X, size.Y, 1, Control.resourceUsage, SurfaceFormat.Color );
            ...
            selectionArea = new Texture2D( BaseGame.Device, size.X - 8, charHeight, 1, Control.resourceUsage, SurfaceFormat.Color );
            ...
            for (int y = 0; y < selectionArea.Height; y++)
            {
                ...
                {
                    pixels[x + y * selectionArea.Width] = new Color( new Vector4( 1f, 1f, 1f, 0.3f ) );
                }
            }
        ...
        public override void Draw( SpriteBatch spriteBatch, float alpha )
        {
            ...
            for (int i = 0; i < lastItem; i++)
            {
                ...
                if (Items[i + startIndex] == selectedItem)
                    spriteBatch.Draw(selectionArea, position + new Vector2(4f, 4f) + new Vector2(0f, charHeight * i), null, dynamicTextColor, 0, Vector2.Zero, 1, SpriteEffects.None, LayerDepth.UI);
