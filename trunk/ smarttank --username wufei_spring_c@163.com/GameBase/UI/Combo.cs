...
namespace GameBase.UI
{
    public class Combo : Control
    {
        ...
        private void Init ()
        {
            ...
            listboxParts[0] = BaseGame.Content.Load<Texture2D>( Path.Combine( Directories.UIContent, "textbox_left" ) );
            listboxParts[1] = BaseGame.Content.Load<Texture2D>( Path.Combine( Directories.UIContent, "textbox_middle" ) );
            listboxParts[2] = BaseGame.Content.Load<Texture2D>( Path.Combine( Directories.UIContent, "textbox_right" ) );
        ...
        public void CreateTextures ()
        {
            ...
            listBackground = new Texture2D( BaseGame.Device, listboxParts[0].Width + listboxRect.Width + listboxParts[2].Width, listHeight, 1, Control.resourceUsage, SurfaceFormat.Color, Control.resourceMode );
            ...
            selectionArea = new Texture2D( BaseGame.Device, listBackground.Width - 8, charHeight, 1, Control.resourceUsage, SurfaceFormat.Color, Control.resourceMode );
        ...
        public override void Draw ( SpriteBatch spriteBatch, float alpha )
        {
            ...
            spriteBatch.Draw( listboxParts[0], position, dynamicColor );
            spriteBatch.Draw( listboxParts[1], listboxRect, dynamicColor );
                ...
                for (int i = 0; i < Items.Count; i++)
                {
                    ...
                    if (ms.LeftButton == ButtonState.Released)
                    {
                        if (bIsOverItem( i ))
                        {
                            ...
                            spriteBatch.Draw( selectionArea, listPosition + new Vector2( 4f, 4f ) + new Vector2( 0f, charHeight * i ), dynamicAreaColor );
                        }
                    }
                }
            }
