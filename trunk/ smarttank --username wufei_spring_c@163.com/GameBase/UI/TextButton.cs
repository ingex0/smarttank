...
namespace GameBase.UI
{
    public class TextButton : Control
    {
        ...
        {
            buttonTexture = BaseGame.Content.Load<Texture2D>( Path.Combine( Directories.UIContent, "button" ) );
        ...
        public override void Draw ( SpriteBatch spriteBatch, float alpha )
        {
            ...
            spriteBatch.Draw( buttonTexture, destRect[0], sourceRect[0], dynamicColor );
            spriteBatch.Draw( buttonTexture, destRect[1], sourceRect[1], dynamicColor );
            spriteBatch.Draw( buttonTexture, destRect[2], sourceRect[2], dynamicColor );

            Color dynamicTextColor = new Color( new Vector4( 0f, 0f, 0f, alpha ) );

            int spacing = (destRect[1].Width - text.Length * fontSize.X) / 2;
            FontManager.DrawInScrnCoord( text, new Vector2( destRect[0].X + destRect[0].Width + spacing, destRect[0].Y + 3f ), Control.fontScale, dynamicTextColor, 0f, FontType.Lucida );
        }
        #endregion
    }
}
